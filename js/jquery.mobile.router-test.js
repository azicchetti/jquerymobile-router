...
$(document).bind("mobileinit",function(){
	...
	*/
	...
	$(document).bind("pagebeforechange", function( e, data ) {
		...
		var hasUiPage=false;
		if ( typeof data.toPage === "string" && !data.options.hasOwnProperty("_jqmrouter")) {
			if (/&ui-page=/.test(data.toPage)){
				hasUiPage=true;
				if (data.toPage.indexOf(window.location.hash)==-1){
					data.toPage=data.toPage.replace(
						window.location.hash.replace(/\?.*$|&ui-page=.*$/g,""),
						window.location.hash.replace(/&ui-page=.*$/g,"")
					);
				}
			}
